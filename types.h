...
public:
    Temporary(Type var_type) : num{get_new_temp()}, var_type{var_type} {}
...
public:
    ...
    Symbol *put(const std::string &name, Type type) {
        return &(scopes.front()[name] = Symbol{name, type});
    }
    ...
    // symbol by that name.
    Address *make_variable(const std::string &name) {
        if (Symbol *s = get(name)) {
            addrs_to_free.push_back(new Variable(s));
